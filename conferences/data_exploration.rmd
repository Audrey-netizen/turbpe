...
Summary text for funsies... or well I think it's kinda redundant but I don't
want to put it in a separate file
```{r}
key_findings_table <- significant_paths %>%
  mutate(
    Interpretation = case_when(
      Std_Coefficient > 0 & P_Value < 0.05 ~ "Significant Positive",
      Std_Coefficient > 0 & P_Value < 0.10 ~ "Marginally Positive",
      Std_Coefficient < 0 & P_Value < 0.05 ~ "Significant Negative",
      Std_Coefficient < 0 & P_Value < 0.10 ~ "Marginally Negative",
      TRUE ~ "Not Significant"
    ),
    Effect_Size = case_when(
      abs(Std_Coefficient) > 0.3 ~ "Large",
      abs(Std_Coefficient) > 0.15 ~ "Medium", 
      abs(Std_Coefficient) > 0.05 ~ "Small",
      TRUE ~ "Very Small"
    )
  )
...
# Again, I've commented them out cuz uh
#write_csv(significant_paths, "sem_significant_paths_widthless.csv")
#write_csv(fit_comparison, "sem_model_comparison_widthless.csv")
#write_csv(key_findings_table, "sem_key_findings_summary.csv")

# Save model summaries
capture.output(summary(focused_fit, standardized = TRUE, fit.measures = TRUE, rsq = TRUE),
               file = "sem_focused_model_summary.txt")

# Save plots
ggsave("bee_vs_flowers_plot.png", p1, width = 8, height = 6, dpi = 300)
ggsave("bee_richness_plot.png", p2, width = 8, height = 6, dpi = 300)
ggsave("traffic_effects_plot.png", p3, width = 8, height = 6, dpi = 300)
ggsave("site_type_comparison.png", p4, width = 8, height = 6, dpi = 300)
ggsave("traffic_flower_richness.png", p5, width = 8, height = 6, dpi = 300)

cat("\n=== ANALYSIS COMPLETE ===\n")
cat("Key files saved:\n")
cat("- sem_significant_paths_widthless.csv: All path coefficients\n")
cat("- sem_model_comparison_widthless.csv: Model fit comparisons\n") 
cat("- sem_key_findings_summary.csv: Interpreted findings\n")
cat("- sem_focused_model_summary.txt: Detailed model summary\n")
cat("- Various PNG plots: Visualizations of key relationships\n")
cat("\nMAIN CONCLUSIONS:\n")
cat("1. Flower diversity strongly supports bee diversity\n")
cat("2. Traffic negatively impacts both bees and flowers\n") 
cat("3. Site type differences exist but are complex\n")
cat("4. Model explains 13.4% of bee richness variation\n")
```
