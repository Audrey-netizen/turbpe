...
and figure presentation, the goal here is for legibility. Based on the version
used for conferences in fall 2025, but will have significant additions in the
repository history. Conferences file is being kept up in the repository
separately for more accessible purposes, I don't think using the history
button is that hard but idk
...
AAAAAAAAAA I did this replicates 1-8 before omg I can't, well will fix that in
due time
```{r}
...
# should be set up to all replicates now
...
```{r}
family_summary <- bee_captures %>%
  count(Family, name = "Count") %>%
  arrange(desc(Count))

print(family_summary)

genus_summary <- bee_captures %>%
...
flowers$Taxonomic_Name <- ifelse(
  is.na(flowers$Taxonomic_Name),
  as.character(flowers$`Species ID`),
...
```{r}
# Create Taxonomic_Name variable/data point by combining Genus and species
# Use remove = FALSE to keep the original Genus/species columns
observation_counts <- flowers %>%
  unite("Taxonomic_Name", Genus, species, sep = " ", remove = FALSE) %>%
  count(Taxonomic_Name, name = "Observation_Count") %>%
  arrange(desc(Observation_Count))

# Go to top 20
top_species <- observation_counts %>%
  slice_max(order_by = Observation_Count, n = 20) %>% # slice_max is the newer way to do top_n
  arrange(desc(Observation_Count))
...
Above function but with changes to accomodate for updated flowers.csv
```{r}
# Clean column names and create the Taxonomy + Treatment categories
flowers_processed <- flowers %>%
  # Remove trailing spaces in column names (e.g., 'Site ' -> 'Site')
  rename_with(trimws) %>%
  # Combine Genus and species into Taxonomic_Name
  unite("Taxonomic_Name", Genus, species, sep = " ", remove = FALSE) %>%
  # Create Treatment_Type from the second letter of Site
  mutate(Treatment_Type = case_when(
    grepl("^.[Tt]", Site) ~ "Treatment",
    grepl("^.[Nn]", Site) ~ "Negative Control", 
    grepl("^.[Pp]", Site) ~ "Positive Control",
...
# Calculate observation counts by treatment
observation_counts_by_treatment <- flowers_processed %>%
  count(Treatment_Type, Taxonomic_Name, name = "Observation_Count") %>%
  arrange(Treatment_Type, desc(Observation_Count))
...
top_species_by_treatment <- observation_counts_by_treatment %>%
  group_by(Treatment_Type) %>%
  slice_max(order_by = Observation_Count, n = 20, with_ties = FALSE) %>%
  arrange(Treatment_Type, desc(Observation_Count)) %>%
  ungroup()
...
for (type in unique(top_species_by_treatment$Treatment_Type)) {
  treatment_plots[[type]] <- create_treatment_plot(
    top_species_by_treatment %>% filter(Treatment_Type == type),
    type
  )
}
...
treatment_plots[["Treatment"]]
treatment_plots[["Negative Control"]]
treatment_plots[["Positive Control"]]
```
...
# gets blurry
comm_matrix_sites <- flowers_processed %>%
...
treatment_groups <- flowers_processed %>%
  filter(Treatment_Type %in% c("Treatment", "Positive Control")) %>%
...
site_counts_neg_treat <- flowers_processed %>%
...
comm_matrix_neg_treat <- flowers_processed %>%
  filter(Treatment_Type %in% c("Treatment", "Negative Control")) %>%
  count(Site, Taxonomic_Name, name = "Count") %>%
  pivot_wider(names_from = Taxonomic_Name, values_from = Count, values_fill = 0) %>%
  column_to_rownames("Site") %>%
...
treatment_groups_neg_treat <- flowers_processed %>%
  filter(Treatment_Type %in% c("Treatment", "Negative Control")) %>%
  distinct(Site, Treatment_Type) %>%
  arrange(Site) %>%
  pull(Treatment_Type)
...
Actually I was just statistically illeterate before, this is quite good yes?
```{r}
...
Same PCoA with negative group
```{r}
# Create a matrix for ALL sites (Treatment, Positive, and Negative)
comm_matrix_all <- flowers_processed %>%
  filter(Treatment_Type %in% c("Treatment", "Positive Control", "Negative Control")) %>%
  count(Site, Taxonomic_Name, name = "Count") %>%
  pivot_wider(names_from = Taxonomic_Name, values_from = Count, values_fill = 0) %>%
  arrange(Site) %>% # Sorting ensures the matrix and groups match up!
  column_to_rownames("Site") %>%
  as.matrix()

# Create the treatment groups variable for all three
treatment_groups_all <- flowers_processed %>%
  filter(Treatment_Type %in% c("Treatment", "Positive Control", "Negative Control")) %>%
  distinct(Site, Treatment_Type) %>%
  arrange(Site) %>% 
  pull(Treatment_Type)

# Run the 3-way PERMANOVA
permanova_all <- adonis2(comm_matrix_all ~ treatment_groups_all, 
                         method = "bray",
                         permutations = 999)

# Calculate Bray-Curtis distance for all sites
dist_matrix_all <- vegdist(comm_matrix_all, method = "bray")

# Perform PCoA
pcoa_result <- cmdscale(dist_matrix_all, eig = TRUE, k = 2)

# Extract scores
pcoa_scores <- as.data.frame(pcoa_result$points)
colnames(pcoa_scores) <- c("PCoA1", "PCoA2")
pcoa_scores$Treatment <- treatment_groups_all
pcoa_scores$Site <- rownames(pcoa_scores)
...
ggplot(pcoa_scores, aes(x = PCoA1, y = PCoA2, color = Treatment)) +
  ...
  stat_ellipse(level = 0.95) +
  geom_text(aes(label = Site), vjust = 1.5, size = 3) +
  # Adding the third color for Negative Control
  scale_color_manual(values = c(
    "Treatment" = "#7BB33D", 
    "Positive Control" = "#273C73",
    "Negative Control" = "#D55E00" # A nice high-contrast orange
  )) +
  labs(x = paste("PCoA1 (", variance_explained[1], "%)"),
       y = paste("PCoA2 (", variance_explained[2], "%)"),
       title = "PCoA: Plant Communities Across All Treatments",
       subtitle = paste("PERMANOVA R² =", round(permanova_all$R2[1], 3),
                        "p =", round(permanova_all$`Pr(>F)`[1], 4))) +
  theme_minimal()
```

Running permanova for the above plot... something is off here
```{r}
# Create the metadata table (one row per site)
metadata_all <- flowers_processed %>%
  filter(Treatment_Type %in% c("Treatment", "Positive Control", "Negative Control")) %>%
  distinct(Site, Treatment_Type) %>%
  arrange(Site) 

# Run the global PERMANOVA test using 'metadata_all' as the data source
permanova_all <- adonis2(comm_matrix_all ~ Treatment_Type, 
                         data = metadata_all, 
                         method = "bray", 
                         permutations = 999)

# Print the results to see the p-value and R2
print(permanova_all)
```

Running pairwise permanovas instead to compare individual controls with the
treatment, that makes more sense now... I think I'm getting it
```{r}
# Treatment vs Positive Control
metadata_tp <- metadata_all %>% filter(Treatment_Type %in% c("Treatment", "Positive Control"))
matrix_tp <- comm_matrix_all[rownames(comm_matrix_all) %in% metadata_tp$Site, ]

perm_tp <- adonis2(matrix_tp ~ Treatment_Type, data = metadata_tp, method = "bray")

# --- Comparison B: Treatment vs. Negative Control ---
metadata_tn <- metadata_all %>% filter(Treatment_Type %in% c("Treatment", "Negative Control"))
matrix_tn <- comm_matrix_all[rownames(comm_matrix_all) %in% metadata_tn$Site, ]

perm_tn <- adonis2(matrix_tn ~ Treatment_Type, data = metadata_tn, method = "bray")

# Print results to compare
print("Treatment vs Positive Control:")
print(perm_tp)

print("Treatment vs Negative Control:")
print(perm_tn)
```

Negative vs positive control pairwise permanova for thoroughness
```{r}
# Filter for just the controls
metadata_pn <- metadata_all %>% 
  filter(Treatment_Type %in% c("Positive Control", "Negative Control"))

# Subset the matrix to match those sites
matrix_pn <- comm_matrix_all[rownames(comm_matrix_all) %in% metadata_pn$Site, ]

# Run PERMANOVA
perm_pn <- adonis2(matrix_pn ~ Treatment_Type, 
                   data = metadata_pn, 
                   method = "bray", 
                   permutations = 999)

print("Positive Control vs Negative Control:")
print(perm_pn)
```


Same figure just grey theming
Plus making it uh shorter for a poster
It works but it's very inelegant since it adds a file to the repository, classic
R
Actually, I have thought about it more, ggsave function makes me want to [do
something harmful to myself], I don't know why I wasn't so upset about it before
I just can't see a way in which this can elegantly be applied in a repository
```{r}
# Define the missing base_theme variable from changes with flowers.csv
base_theme <- theme_minimal()

# Re-do plot
p_pcoa <- ggplot(pcoa_scores, aes(x = PCoA1, y = PCoA2, color = Treatment)) +
  geom_point(size = 3) +
  stat_ellipse(level = 0.95) +
  geom_text(aes(label = Site), vjust = 1.5, size = 3) +
  
  # Separate Negative Control colour if I waanna include it in the future
  scale_color_manual(values = c(
    "Treatment" = "#7BB33D", 
    "Positive Control" = "#273C73",
  ...
  labs(
    ...
    y = paste("PCoA2 (", variance_explained[2], "%)"),
    title = "PCoA: Treatment vs Controls",
    subtitle = paste("PERMANOVA R² =", round(permanova_all$R2[1], 3),
                     "p =", round(permanova_all$`Pr(>F)`[1], 4))
  ) +
  ...
  base_theme + 
  theme(
    panel.background = element_rect(fill = "#bbbbbbff", colour = NA),
    plot.background = element_rect(fill = "#bbbbbbff", colour = NA),
...
```{r}
flower_summary <- flowers_processed %>%
...
```{r}
# For da bees
bee_diversity_indices <- bee_captures %>%
...
flower_diversity_indices <- flowers_processed %>%
  filter(Replicate >= 1 & Replicate <= 8) %>%
...
graaaaa this should be here in all files for continuity purposes, I am very
dissapointed in younger me, but alas
anyways this writes sem_prepared csv files which I need for future analyses. It
was in the repository before but since my csv files got updated yknow I needed
to remake them. This was the only chunk of write_csv stuff that was not in my
conferences 2025 file and I really have no idea why
```{r}
cat("\nSummary statistics:\n")
...
New note: what the hell is this? How did I go about making a csv file that's
"friendly for SEM analysis" but it clearly has to have been made with some sort
of programming to put together? There should be some code previously for this, I
assume that was in a previous document. Regardless, horrifically sloppy from me.
```{r}
...
Words summary, hope it helps! (This is so useless lol)
```{r}
...
```{r}
# Had to be re-updated here cuz of working through flowers.csv again
colnames(flowers) <- trimws(colnames(flowers))

# Create a Taxonomic_Name variable directly from the Genus and species columns
# This replaces the entire 'merge' and 'ifelse' block you had before
flowers_with_taxonomy <- flowers %>%
  mutate(Taxonomic_Name = paste(Genus, species))

# Clean up any "NA NA" strings if Genus or species were missing, this is WIP
flowers_with_taxonomy$Taxonomic_Name <- gsub("NA NA", "Unknown", flowers_with_taxonomy$Taxonomic_Name)

# Now check the result
head(flowers_with_taxonomy$Taxonomic_Name)
```
...
# Flowers
flower_summary <- flowers %>%
  # Fix the 'Site ' column name and other trailing spaces
  rename_with(trimws) %>% 
  # Create the missing Taxonomic_Name column
  mutate(Taxonomic_Name = paste(Genus, species)) %>%
...
# For flowers
flower_diversity_indices <- flowers %>%
  rename_with(trimws) %>%
  mutate(Taxonomic_Name = paste(Genus, species)) %>%
  filter(Replicate >= 1 & Replicate <= 10) %>%
  group_by(Site, Replicate, Taxonomic_Name) %>%
  ...
  group_by(Site, Replicate) %>%
  summarise(
    flower_shannon = ifelse(n() > 1, vegan::diversity(count, index = "shannon"), 0),
    .groups = 'drop'
  )
...
Same boxplot just changing background colour to fit a poster better, plus making
titling more visible
Plus making it uh shorter? Hope this works
It works but it's very inelegant since it adds a file to the repository, classic
R
```{r}
p_mean_comparison <- ggplot(analysis_data_mean, aes(x = Site_Type, y = bee_abundance, fill = Site_Type)) +
  ...
  coord_cartesian(ylim = c(0, max(tukey_plot_data$y.position) * 1.05)) +
  ...
  theme(
    ...
    # Title font bigger 22, and centered
    plot.title = element_text(hjust = 0.5, size = 22, face = "bold"),
...
```{r}
# Cleaning data that's scuffed now cuz of flowers.csv again
plant_key <- plant_IDs %>%
  distinct(ID, .keep_all = TRUE) %>%
  mutate(Full_Name = paste(Genus, Species))

flowers <- flowers %>%
  # Fixes the "Site " trailing space in the header
  rename_with(trimws) %>%
  # Create the Taxonomic_Name column by combining Genus and species
  unite("Taxonomic_Name", Genus, species, sep = " ", remove = FALSE) %>%
  # Now create the Treatment_Type using the cleaned Site column
  mutate(Treatment_Type = case_when(
    grepl("^.[Tt]", Site) ~ "Treatment",
    grepl("^.[Nn]", Site) ~ "Negative Control", 
    grepl("^.[Pp]", Site) ~ "Positive Control",
...
observation_counts_by_treatment <- flowers %>%
  count(Treatment_Type, Taxonomic_Name, name = "Observation_Count") %>%
  arrange(Treatment_Type, desc(Observation_Count))
...
Why did I not show mean +/- SD of these until now?
```{r}
# Calculate Mean and SD for each group
summary_stats <- analysis_data_mean %>%
  group_by(Site_Type) %>%
  summarize(
    mean_abundance = mean(bee_abundance),
    sd_abundance = sd(bee_abundance),
    n = n() # Useful to see sample size
  )
...
flower_richness_per_site <- flowers %>%
...
bee_captures_clean <- bee_captures_clean %>%
  mutate(
...
head(bee_captures_clean %>% select(Family, Genus, Species, taxonomic_id, species_name, Site, site_category))
```

Sorting out genus of bees into the site categories
```{r}
# For negative controls
genus_n_sites <- bee_captures_clean %>%
...
# For treatments
genus_t_sites <- bee_captures_clean %>%
...
genus_p_sites <- bee_captures_clean %>%
...
Stats test of the above? Why is this here? Will likely cut it later once I 
figure things out more
```{r}
# Get the slopes (trends) for total_flowers for each Site_Type
slopes <- emtrends(model_full, ~ Site_Type, var = "total_flowers")
...
```{r}
# Model for Richness using Log-Log relationship
model_richness <- glmmTMB(
  bee_richness ~ log10(flower_richness + 1) * Site_Type + (1 | Site),
  data = analysis_data,
  family = nbinom1(link = "log")
)


# Get the slopes for each Site_Type
richness_slopes <- emtrends(model_richness, 
                            specs = ~ Site_Type, 
                            var = "log10(flower_richness + 1)")

print(richness_slopes)
```
...
# 1. Fit the model using log10 of traffic to match the plot's X-axis
model_traffic <- glmmTMB(
  bee_abundance ~ log10(traffic_metric + 1) * Site_Type + (1 | Site),
  data = analysis_data,
  family = nbinom1(link = "log")
)

# 2. Extract the trends (slopes) for each Site_Type
traffic_slopes <- emtrends(model_traffic, 
                           specs = ~ Site_Type, 
                           var = "log10(traffic_metric + 1)")

# 3. View the results
print(traffic_slopes)
```
...
Flower richness vs roadside traffic associations... this one had to be
significantly troubleshooted compared to the previous model that yknow I figure
would be similar enough in code now but I guess not...
```{r}
# Create the site type function 
# This was a terminal fix for the previous issue I am just reapplying here
create_site_type <- function(site_code) {
  site_code <- toupper(trimws(as.character(site_code)))
  if (grepl("P", site_code)) return("Positive Control")
  if (grepl("N", site_code)) return("Negative Control") 
  if (grepl("T", site_code)) return("Treatment")
  return("Other")
}
...
sem_data_widthless <- bee_summary %>%
  # Fix Site names on the fly during joins
  mutate(Site = trimws(Site)) %>%
  full_join(flower_summary %>% mutate(Site = trimws(Site)), by = c("Site", "Replicate")) %>%
  left_join(abiotic_summary %>% mutate(Site = trimws(Site)), by = c("Site", "Replicate")) %>%
  left_join(bee_diversity_indices %>% mutate(Site = trimws(Site)), by = c("Site", "Replicate")) %>%
  left_join(flower_diversity_indices %>% mutate(Site = trimws(Site)), by = c("Site", "Replicate")) %>%
  # CREATE ALL NECESSARY COLUMNS HERE
  mutate(
    Site_Type = sapply(Site, create_site_type),
    # Ensure total_traffic exists (summing across traffic types)
    total_traffic = replace_na(total_traffic_passenger, 0) + 
                    replace_na(total_traffic_bike, 0) + 
                    replace_na(total_traffic_truck, 0),
    # Calculate the mean traffic the model is looking for
    mean_traffic = total_traffic / 3,
    # Fill NAs for richness/abundance so the model doesn't drop rows
    across(c(bee_abundance, bee_richness, total_flowers, flower_richness), ~replace_na(., 0)),
    bee_shannon = replace_na(bee_shannon, 0),
    flower_shannon = replace_na(flower_shannon, 0)
  )

# Verify one more time
cat("Checking columns:", colnames(sem_data_widthless), "\n")
table(sem_data_widthless$Site_Type)
```

```{r}
# Prepare the final model data
analysis_data <- sem_data_widthless %>%
  filter(!is.na(mean_traffic)) %>%
  mutate(
    Site_Type = factor(Site_Type),
    # Scaling fixes the "Convergence" error you were seeing earlier
    log_traffic_scaled = scale(log(mean_traffic + 1))
  )

# Run the model
model_full_traffic_flowers <- glmmTMB(
  flower_richness ~ log_traffic_scaled * Site_Type + (1 | Site),
  data = analysis_data,
  family = nbinom1(link = "log")
)

# This should now work!
summary(model_full_traffic_flowers)
```

Stats significance check
```{r}

# Extract the trends using the scaled variable name
flower_traffic_slopes <- emtrends(model_full_traffic_flowers, 
                                  specs = ~ Site_Type, 
                                  var = "log_traffic_scaled")

# View the results
print(flower_traffic_slopes)
```
...
Adding in analyses to compare road width to bee abundance/diversity and plant
diversity
```{r}
# Process Bee Abundance
# We count total bees per Site/Date, then average those daily totals for each Site
site_abundance <- bee_captures %>%
  group_by(Site, Date) %>%
  summarise(daily_total = n(), .groups = "drop") %>%
  group_by(Site) %>%
  summarise(avg_daily_abundance = mean(daily_total))

# Clean Road Width Data
# Using backticks for "Road Width (m)" because read_csv preserves spaces
road_data_clean <- road_widths %>%
  select(Site, `Road Width (m)`) %>% 
  filter(!is.na(Site))

# Merge and Filter Site Types
# We only keep Treatment (T) and Negative Control (N) sites
final_data <- inner_join(site_abundance, road_data_clean, by = "Site") %>%
  mutate(SiteType = substr(Site, 2, 2)) %>%
  filter(SiteType %in% c("T", "N"))

# Run the Three Models

# --- Model 1: Treatment sites only ---
# (Sites like DT1, GT3 where the second character is 'T')
treatment_df <- final_data %>% filter(SiteType == "T")
model_treatment <- lm(avg_daily_abundance ~ `Road Width (m)`, data = treatment_df)

print("--- SUMMARY: TREATMENT SITES ONLY ---")
summary(model_treatment)

# --- Model 2: Negative control sites only ---
# (Sites like DN3, WN2 where the second character is 'N')
negative_df <- final_data %>% filter(SiteType == "N")
model_negative <- lm(avg_daily_abundance ~ `Road Width (m)`, data = negative_df)

print("--- SUMMARY: NEGATIVE CONTROL SITES ONLY ---")
summary(model_negative)

# --- Model 3: Combined (Treatment + Negative Control) ---
model_combined <- lm(avg_daily_abundance ~ `Road Width (m)`, data = final_data)

print("--- SUMMARY: COMBINED SITES ---")
summary(model_combined)
```

Line graphs for these
```{r}
# Prepare the plotting data (assuming final_data from previous step)
# Data is sorted by Road Width so the 'broken line' connects points in order
plot_data <- final_data %>%
  arrange(`Road Width (m)`)

# 1. Plot: Treatment Sites
ggplot(subset(plot_data, SiteType == "T"), aes(x = `Road Width (m)`, y = avg_daily_abundance)) +
  geom_line(color = "blue", alpha = 0.4) + # The 'broken line' connecting points
  geom_point(color = "blue", size = 3) +
  geom_smooth(method = "lm", color = "darkblue", se = TRUE) + # The linear trend
  labs(title = "Treatment Sites: Road Width vs Bee Abundance",
       subtitle = "Blue line = connected data points; Dark blue = linear trend",
       y = "Avg Daily Abundance", x = "Road Width (m)") +
  theme_minimal()

# 2. Plot: Negative Control Sites
ggplot(subset(plot_data, SiteType == "N"), aes(x = `Road Width (m)`, y = avg_daily_abundance)) +
  geom_line(color = "red", alpha = 0.4) +
  geom_point(color = "red", size = 3) +
  geom_smooth(method = "lm", color = "darkred", se = TRUE) +
  labs(title = "Negative Control Sites: Road Width vs Bee Abundance",
       y = "Avg Daily Abundance", x = "Road Width (m)") +
  theme_minimal()

# 3. Plot: Combined
ggplot(plot_data, aes(x = `Road Width (m)`, y = avg_daily_abundance)) +
  geom_point(aes(color = SiteType), size = 3) +
  geom_smooth(method = "lm", color = "black", se = TRUE) +
  labs(title = "Combined Sites: Road Width vs Bee Abundance",
       subtitle = "Black line represents the overall linear trend",
       y = "Avg Daily Abundance", x = "Road Width (m)") +
  scale_color_manual(values = c("N" = "red", "T" = "blue"), labels = c("Negative", "Treatment")) +
  theme_minimal()
```

Now to look at bee genus diversity
```{r}
# Process Bee Diversity
# We count unique Genera per Site and divide by the count of unique Dates per Site
site_diversity <- bee_captures %>%
  group_by(Site) %>%
  summarise(
    unique_genera = n_distinct(Genus, na.rm = TRUE),
    sampling_days = n_distinct(Date),
    avg_diversity = unique_genera / sampling_days,
    .groups = "drop"
  )
...
# Merge and Filter Site Types
final_diversity_data <- inner_join(site_diversity, road_data_clean, by = "Site") %>%
  mutate(SiteType = substr(Site, 2, 2)) %>%
  filter(SiteType %in% c("T", "N"))

# Run the Three Models (Diversity)

# Model 1: Treatment sites
model_div_treatment <- lm(avg_diversity ~ `Road Width (m)`, 
                          data = subset(final_diversity_data, SiteType == "T"))
print("--- SUMMARY: DIVERSITY AT TREATMENT SITES ---")
summary(model_div_treatment)

# Model 2: Negative control sites
model_div_negative <- lm(avg_diversity ~ `Road Width (m)`, 
                         data = subset(final_diversity_data, SiteType == "N"))
print("--- SUMMARY: DIVERSITY AT NEGATIVE CONTROL SITES ---")
summary(model_div_negative)

# Model 3: Combined
model_div_combined <- lm(avg_diversity ~ `Road Width (m)`, data = final_diversity_data)
print("--- SUMMARY: DIVERSITY COMBINED ---")
summary(model_div_combined)

# Visualization have data sorted by Road Width for the broken line
plot_div_data <- final_diversity_data %>% arrange(`Road Width (m)`)

# Plot: Treatment Sites
ggplot(subset(plot_div_data, SiteType == "T"), aes(x = `Road Width (m)`, y = avg_diversity)) +
  geom_line(color = "blue", alpha = 0.4) +
  geom_point(color = "blue", size = 3) +
  geom_smooth(method = "lm", color = "darkblue", se = TRUE) +
  labs(title = "Treatment Sites: Road Width vs Genus Diversity",
       y = "Avg Diversity (Unique Genera / Days)", x = "Road Width (m)") +
  theme_minimal()

# Plot: Negative Control Sites
ggplot(subset(plot_div_data, SiteType == "N"), aes(x = `Road Width (m)`, y = avg_diversity)) +
  geom_line(color = "red", alpha = 0.4) +
  geom_point(color = "red", size = 3) +
  geom_smooth(method = "lm", color = "darkred", se = TRUE) +
  labs(title = "Negative Control Sites: Road Width vs Genus Diversity",
       y = "Avg Diversity (Unique Genera / Days)", x = "Road Width (m)") +
  theme_minimal()

# Plot: Combined
ggplot(plot_div_data, aes(x = `Road Width (m)`, y = avg_diversity)) +
  geom_point(aes(color = SiteType), size = 3) +
  geom_smooth(method = "lm", color = "black", se = TRUE) +
  labs(title = "Combined Sites: Road Width vs Genus Diversity",
       y = "Avg Diversity (Unique Genera / Days)", x = "Road Width (m)") +
  scale_color_manual(values = c("N" = "red", "T" = "blue"), labels = c("Negative", "Treatment")) +
  theme_minimal()
```

Same again now for flower diversity
```{r}
# Process Flower Diversity
colnames(flowers) <- trimws(colnames(flowers))

site_flower_diversity <- flowers %>%
  # Create a unique Taxa column (Genus + species)
  mutate(Taxa = paste(Genus, species)) %>%
  group_by(Site) %>%
  summarise(
    unique_taxa = n_distinct(Taxa, na.rm = TRUE),
    sampling_days = n_distinct(Date),
    avg_flower_diversity = unique_taxa / sampling_days,
    .groups = "drop"
  )

# Clean Road Width Data (this can be cut right cuz it's already been done in
# earlier code chunks?)
road_data_clean <- road_widths %>%
  select(Site, `Road Width (m)`) %>% 
  filter(!is.na(Site))
...
# Merge and Filter Site Types
final_flower_data <- inner_join(site_flower_diversity, road_data_clean, by = "Site") %>%
  mutate(SiteType = substr(Site, 2, 2)) %>%
  filter(SiteType %in% c("T", "N"))

# Run the Three Models (Flower Diversity)

# Model 1: Treatment sites
model_fl_treatment <- lm(avg_flower_diversity ~ `Road Width (m)`, 
                          data = subset(final_flower_data, SiteType == "T"))
print("--- SUMMARY: FLOWER DIVERSITY AT TREATMENT SITES ---")
summary(model_fl_treatment)

# Model 2: Negative control sites
model_fl_negative <- lm(avg_flower_diversity ~ `Road Width (m)`, 
                         data = subset(final_flower_data, SiteType == "N"))
print("--- SUMMARY: FLOWER DIVERSITY AT NEGATIVE CONTROL SITES ---")
summary(model_fl_negative)

# Model 3: Combined
model_fl_combined <- lm(avg_flower_diversity ~ `Road Width (m)`, data = final_flower_data)
print("--- SUMMARY: FLOWER DIVERSITY COMBINED ---")
summary(model_fl_combined)

# Visualization
# Ensure data is sorted by Road Width for the broken line
plot_fl_data <- final_flower_data %>% arrange(`Road Width (m)`)

# Plot: Treatment Sites
ggplot(subset(plot_fl_data, SiteType == "T"), aes(x = `Road Width (m)`, y = avg_flower_diversity)) +
  geom_line(color = "blue", alpha = 0.4) +
  geom_point(color = "blue", size = 3) +
  geom_smooth(method = "lm", color = "darkblue", se = TRUE) +
  labs(title = "Treatment Sites: Road Width vs Flower Diversity",
       y = "Avg Diversity (Taxa / Days)", x = "Road Width (m)") +
  theme_minimal()

# Plot: Negative Control Sites
ggplot(subset(plot_fl_data, SiteType == "N"), aes(x = `Road Width (m)`, y = avg_flower_diversity)) +
  geom_line(color = "red", alpha = 0.4) +
  geom_point(color = "red", size = 3) +
  geom_smooth(method = "lm", color = "darkred", se = TRUE) +
  labs(title = "Negative Control Sites: Road Width vs Flower Diversity",
       y = "Avg Diversity (Taxa / Days)", x = "Road Width (m)") +
  theme_minimal()

# Plot: Combined
ggplot(plot_fl_data, aes(x = `Road Width (m)`, y = avg_flower_diversity)) +
  geom_point(aes(color = SiteType), size = 3) +
  geom_smooth(method = "lm", color = "black", se = TRUE) +
  labs(title = "Combined Sites: Road Width vs Flower Diversity",
       y = "Avg Diversity (Taxa / Days)", x = "Road Width (m)") +
  scale_color_manual(values = c("N" = "red", "T" = "blue"), labels = c("Negative", "Treatment")) +
  theme_minimal()
```